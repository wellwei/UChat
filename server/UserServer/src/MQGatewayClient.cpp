...
bool MQGatewayClient::initialize(const std::string& serverAddress) {
    ...
    if (!m_channel) {
        LOG_ERROR("创建 gRPC 通道失败：{}", serverAddress);
        return false;
    }
    ...
    if (!m_channel->WaitForConnected(deadline)) {
        LOG_ERROR("连接 MQGateway 超时：{}", serverAddress);
        return false;
    }

    m_connected = true;
    LOG_INFO("成功连接到 MQGateway: {}", serverAddress);
